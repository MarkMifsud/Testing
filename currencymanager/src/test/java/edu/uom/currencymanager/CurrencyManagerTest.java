...
import currencymanager.currenciesTest.CurrencyTest;
import currencymanager.currenciesTest.CurrencyDatabase;
import currencymanager.currenciesTest.ExchangeRate;
...
public class CurrencyManager {
        ...
        while (!exit) {
            ...
            switch (choice) {
                case 0:
                    ...
                    break;
                case 1:
                    List<CurrencyTest> currencies = manager.currencyDatabase.getCurrencies();
                    System.out.println("\nAvailable Currencies\n--------------------");
                    for (CurrencyTest currency : currencies) {
    ...
    public List<ExchangeRate> getMajorCurrencyRates() throws Exception {
        ...
        List<CurrencyTest> currencies = currencyDatabase.getMajorCurrencies();

        for (CurrencyTest src : currencies) {
            for (CurrencyTest dst : currencies) {
                if (src != dst) {
    ...
    public void addCurrency(String code, String name, boolean major) throws Exception {
        ...
        currencyDatabase.addCurrency(new CurrencyTest(code,name,major));
